...
ORDER BY order_count DESC, ship_address;
-- 2.4
...
SELECT territory_description, r.region_description FROM territories t
JOIN regions r ON t.region_id = r.region_id
WHERE NOT territory_id IN (
    SELECT DISTINCT territory_id FROM employees_territories
)
...
SELECT company_name, address, city, region, postal_code, country FROM suppliers s
UNION
...
-- in descending order.
SELECT company_name, SUM(od.quantity) FROM order_details od
